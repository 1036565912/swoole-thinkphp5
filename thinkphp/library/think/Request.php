...
class Request
{
    ...
    {
            //支持swoole http服务  可以实现兼容模式和pathinfo模式
            if(isset($_SERVER['PATH_INFO']) && $_SERVER['PATH_INFO'] != '/'){
                return $this->pathinfo = ltrim($_SERVER['PATH_INFO'],'/');
            }
        //if (is_null($this->pathinfo)) {
            ...
            } elseif ($this->isCli()) { //swoole 走的cli模式 无法直接使用pathinfo模式
                // CLI模式下 index.php module/controller/action/params/...
                $_SERVER['PATH_INFO'] = isset($_SERVER['argv'][1]) ? $_SERVER['argv'][1] : '';
            }
            // 分析PATHINFO信息 走apache的时候pathinfo可以被访问  nginx需要进行重写 变成兼容模式 swoole 只能修改框架
